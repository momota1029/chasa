//! Set-like predicates for item parsers.
//!
//! [`crate::parser::item::one_of`] and [`crate::parser::item::none_of`] accept any [`ItemSet`].
//! This module provides a small trait plus combinators to build sets out of common Rust types:
//!
//! - `&str` and `char` for `char` inputs
//! - range types like `..`, `..=`, `a..b`, etc.
//! - boolean predicates (`Fn(Item) -> bool`)
//!
//! You can combine sets via [`ItemSet::cap`] (intersection), [`ItemSet::and`] (union),
//! [`ItemSet::sub`] (difference), and [`ItemSet::complement`].
//!
//! ```rust
//! use chasa::parser::item::set::ItemSet;
//!
//! let letters = ('a'..='z').and('A'..='Z');
//! assert!(letters.has(&'q'));
//! assert!(letters.has(&'Q'));
//! assert!(!letters.has(&'9'));
//! ```

use ::std::ops;

/// A set-like predicate over items.
///
/// The associated type [`ItemSet::Copyable`] exists because some set expressions (like ranges)
/// are not `Copy`, but `one_of`/`none_of` store a copyable representation.
pub trait ItemSet<Item> {
    type Copyable: ItemSet<Item>;

    /// Return whether `item` is in this set.
    fn has(&self, item: &Item) -> bool;
    /// Convert into a `Copyable` representation.
