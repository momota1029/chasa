...
/// ```
/// use chasa::*;
/// assert_eq!(pure_or(Some("first"), string("second".chars(), "second")).parse_easy("second".chars()).ok(), Some("first"));
/// assert_eq!(pure_or(None, string("second".chars(), "second")).parse_easy("second".chars()).ok(), Some("second"))
/// ```
...
/// ```
/// use chasa::*;
/// assert_eq!(any.bind(|c| char(c)).parse_easy("aa".chars()).ok(), Some('a'));
/// assert_eq!(char('b').bind(|_| char('a')).parse_easy("aa".chars()).ok(), None);
/// assert_eq!(any.bind(|c| char(c)).parse_easy("ab".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(any.and(any).parse_easy("aa".chars()).ok(), Some(('a','a')))
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(any.left(any).parse_easy("ab".chars()).ok(), Some('a'));
/// assert_eq!(any.left(char('a')).parse_easy("ab".chars()).ok(), None);
/// assert_eq!(string("chasa".chars(), ()).left(char(':')).parse_easy("chasa: parser combinator".chars()).ok(), Some(()));
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(any.right(any).parse_easy("ab".chars()).ok(), Some('b'));
/// assert_eq!(char('b').right(any).parse_easy("ab".chars()).ok(), None);
/// assert_eq!(ws.right(string("chasa".chars(), ())).parse_easy("   chasa".chars()).ok(), Some(()));
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(any.between(char('('), char(')')).parse_easy("(a)".chars()).ok(), Some('a'));
/// assert_eq!(char('a').between(char('('), char(')')).parse_easy("(a".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
///     })
/// }
/// assert_eq!(parser.parse_easy("abc".chars()).ok(), Some(10));
/// assert_eq!(parser.parse_easy("bcaa".chars()).ok(), Some(20));
/// assert_eq!(parser.parse_easy("bcabacca".chars()).ok(), Some(30));
/// assert_eq!(parser.parse_easy("ba".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// });
/// assert_eq!(p.to_ref().parse_easy("abc".chars()).ok(), Some(true));
/// assert_eq!(p.to_ref().parse_easy("cba".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(char('a').or(char('b')).parse_easy("aa".chars()).ok(), Some('a'));
/// assert_eq!(char('a').or(char('b')).parse_easy("bb".chars()).ok(), Some('b'));
/// assert_eq!(char('a').right(char('b')).or(char('b').right(char('b'))).parse_easy("bb".chars()).ok(), Some('b'));
/// assert_eq!(char('b').right(char('b')).or(char('b').right(char('a'))).parse_easy("ba".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(char('a').or_not().parse_easy("aa".chars()).ok(), Some(Some('a')));
/// assert_eq!(char('a').or_not().parse_easy("bb".chars()).ok(), Some(None));
/// assert_eq!(char('a').right(char('b')).or_not().parse_easy("bb".chars()).ok(), Some(None));
/// assert_eq!(char('b').right(char('b')).or_not().parse_easy("ba".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// use chasa::*;
/// assert_eq!(char('b').right(char('b')).or(char('b').right(char('a'))).parse_easy("ba".chars()).ok(), None);
/// assert_eq!(char('b').right(char('b')).cut().or(char('b').right(char('a'))).parse_easy("ba".chars()).ok(), Some('a'));
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(char('a').ranged().parse_easy("a".chars()).ok(), Some(('a',0,1)));
/// assert_eq!(string("abcd".chars(),()).ranged().parse_easy("abcd".chars()).ok(), Some(((),0,4)));
/// assert_eq!(ws.right(string("abcd".chars(),()).ranged()).parse_easy("    abcd".chars()).ok(), Some(((),4,8)))
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(char('a').and(char('a')).parse_easy("a".chars()).ok(), None);
/// assert_eq!(before(char('a')).and(char('a')).parse_easy("a".chars()).ok(), Some(('a','a')));
/// assert_eq!(char('a').and(char('b')).parse_easy("ab".chars()).ok(), Some(('a','b')));
/// assert_eq!(before(char('a')).and(char('b')).parse_easy("ab".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(char('a').and(char('a')).parse_easy("aa".chars()).ok(), Some(('a','a')));
/// assert_eq!(not_followed_by(char('a'),'a').and(char('a')).parse_easy("aa".chars()).ok(), None);
/// assert_eq!(not_followed_by(char('b'),'b').and(char('a')).parse_easy("a".chars()).ok(), Some(((),'a')));
/// assert_eq!(not_followed_by(char('b'),'b').and(any).parse_easy("b".chars()).ok(), None);
/// assert_eq!(not_followed_by(char('b').and(char('a')),'b').and(any).parse_easy("bb".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// let d = d.to_ref();
/// assert_eq!(d.fold(0,|a,b| a+b).parse_easy("12345".chars()).ok(), Some(15));
/// assert_eq!(d.fold(0,|a,b| a+b).parse_easy("".chars()).ok(), Some(0));
/// assert_eq!(d.fold(0,|a,b| a+b).and(d).parse_easy("12345".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// let d = one_of("0123456789".chars()).and_then(|c: char| c.to_string().parse::<usize>().map_err(prim::Error::Message));
/// let d = d.to_ref();
/// assert_eq!(d.fold1(0,|a,b| a+b).parse_easy("12345".chars()).ok(), Some(15));
/// assert_eq!(d.fold1(0,|a,b| a+b).parse_easy("".chars()).ok(), None);
/// assert_eq!(d.fold1(0,|a,b| a+b).and(d).parse_easy("12345".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// let d = one_of("0123456789".chars()).and_then(|c: char| c.to_string().parse::<usize>().map_err(prim::Error::Message));
/// let d = d.to_ref();
/// assert_eq!(d.sep(0,|a,b| a+b, char(',')).parse_easy("1,2,3,4,5".chars()).ok(), Some(15));
/// assert_eq!(d.sep(0,|a,b| a+b, char(',')).parse_easy("".chars()).ok(), Some(0));
/// assert_eq!(d.sep(0,|a,b| a+b, char(',')).and(char(',').right(d)).parse_easy("1,2,3,4,5,6".chars()).ok(), None);
/// assert_eq!(d.sep(0,|a,b| a+b, char(',')).and(char(',')).parse_easy("1,2,3,4,".chars()).ok(), Some((10, ',')));
/// ```
#[derive(Clone, Copy)]
pub struct Sep<T, P1, P2, F> {
    pub(crate) init: T,
    pub(crate) p: P1,
    pub(crate) sep: P2,
    pub(crate) succ: F,
...
/// let d = one_of("0123456789".chars()).and_then(|c: char| c.to_string().parse::<usize>().map_err(prim::Error::Message));
/// let d = d.to_ref();
/// assert_eq!(d.sep1(0,|a,b| a+b, char(',')).parse_easy("1,2,3,4,5".chars()).ok(), Some(15));
/// assert_eq!(d.sep1(0,|a,b| a+b, char(',')).parse_easy("".chars()).ok(), None);
/// assert_eq!(d.sep1(0,|a,b| a+b, char(',')).and(char(',').right(d)).parse_easy("1,2,3,4,5,6".chars()).ok(), None);
/// assert_eq!(d.sep1(0,|a,b| a+b, char(',')).and(char(',')).parse_easy("1,2,3,4,".chars()).ok(), Some((10, ',')));
/// ```
#[derive(Clone, Copy)]
pub struct Sep1<T, P1, P2, F> {
    pub(crate) init: T,
    pub(crate) p: P1,
    pub(crate) sep: P2,
    pub(crate) succ: F,
    ...
    for Sep1<T, P1, P2, F>
{
    ...
    fn run_once(self, cont: ICont<I, C, S, M>) -> IResult<T, I, S, M> {
        ...
        self.p
            .run(ICont { ok, config, drop })
            .and_then(|(o, ok)| run_left_sep((self.succ)(self.init, o), self.p, self.sep, self.succ, ok.to_cont(config, drop)))
    }
}
impl<I: Input + Clone, C, S: Clone, M: Cb, T: Clone, P1: Parser<I, C, S, M>, P2: Parser<I, C, S, M>, F: Fn(T, P1::Output) -> T> Parser<I, C, S, M>
    for Sep1<T, P1, P2, F>
...
/// ```
/// use chasa::*;
/// assert_eq!(any.extend(String::new()).parse_easy("abcde".chars()).ok(), Some("abcde".to_string()));
/// assert_eq!(char('a').extend(String::new()).parse_easy("aaabbb".chars()).ok(), Some("aaa".to_string()));
/// assert_eq!(char('a').extend(String::new()).parse_easy("b".chars()).ok(), Some("".to_string()));
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(any.extend1(String::new()).parse_easy("abcde".chars()).ok(), Some("abcde".to_string()));
/// assert_eq!(char('a').extend1(String::new()).parse_easy("aaabbb".chars()).ok(), Some("aaa".to_string()));
/// assert_eq!(char('a').extend1(String::new()).parse_easy("b".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
// sepが成功した後に本体がempty failしても元に戻る仕組み
fn run_left_extend_sep<I: Input + Clone, C, S: Clone, M: Cb, T: Extend<P1::Output>, P1: Parser<I, C, S, M>, P2: Parser<I, C, S, M>>(
    ...
    match run_drop(sep.to_ref(), ICont { ok, config, drop }, ()) {
        ...
        (Ok((_, ok)), _) => {
            ...
            match run_drop(p.to_ref(), ok.to_cont(config, drop), (input, state)) {
                ...
                (Ok((o, ok)), _) => {
                    t.extend(once(o));
                    run_left_extend_sep(t, p, sep, ok.to_cont(config, drop))
                },
            }
        },
    }
    ...
    fn run_once(mut self, cont: ICont<I, C, S, M>) -> IResult<O, I, S, M> {
        ...
        match run_drop(self.p.to_ref(), ICont { ok, config, drop }, (input, state)) {
            (Ok((o, ok)), _) => {
                ...
                run_left_extend_sep(self.init, self.p, self.sep, ok.to_cont(config, drop))
            },
            (Err(e), None) => Err(e),
            (Err(e), Some((input, state))) => Ok((self.init, IOk { input, state, cutted, err: Some(e) })),
        }
    ...
    fn run(&self, cont: ICont<I, C, S, M>) -> IResult<O, I, S, M> {
        ...
        match run_drop(self.p.to_ref(), ICont { ok, config, drop }, (input, state)) {
            (Ok((o, ok)), _) => {
                ...
                init.extend(once(o));
                run_left_extend_sep(init, self.p.to_ref(), self.sep.to_ref(), ok.to_cont(config, drop))
            },
            (Err(e), None) => Err(e),
            (Err(e), Some((input, state))) => Ok((self.init.clone(), IOk { input, state, cutted, err: Some(e) })),
        }
...
#[derive(Clone, Copy)]
pub struct ExtendSep1<O, P1, P2> {
    pub(crate) init: O,
    pub(crate) p: P1,
    pub(crate) sep: P2,
}
impl<I: Input + Clone, C, S: Clone, M: Cb, P1: Parser<I, C, S, M>, P2: Parser<I, C, S, M>, O: Extend<P1::Output>> ParserOnce<I, C, S, M>
    for ExtendSep1<O, P1, P2>
{
    type Output = O;
    #[inline]
    fn run_once(mut self, cont: ICont<I, C, S, M>) -> IResult<O, I, S, M> {
...
///     Either::Right(_) => Ok(n)
/// })));
/// assert_eq!(p.to_ref().parse_easy("(12345)".chars()).ok(), Some(15));
/// assert_eq!(p.to_ref().parse_easy("(12)345)".chars()).ok(), Some(3));
/// assert_eq!(p.to_ref().parse_easy("()".chars()).ok(), Some(0));
/// assert_eq!(p.to_ref().parse_easy("(12a345)".chars()).ok(), None);
/// ```
#[derive(Clone, Copy)]
...
/// ```
/// use chasa::*;
/// assert_eq!(any.many::<Vec<_>>().parse_easy("1234".chars()).ok(), Some(vec!['1','2','3','4']));
/// assert_eq!(any.many::<String>().parse_easy("1234".chars()).ok(), Some("1234".to_string()));
/// assert_eq!(char('a').many::<String>().parse_easy("aaabaaba".chars()).ok(), Some("aaa".to_string()));
/// assert_eq!(char('a').many::<String>().parse_easy("".chars()).ok(), Some("".to_string()));
/// ```
...
/// ```
/// use chasa::*;
/// assert_eq!(any.many1::<Vec<_>>().parse_easy("1234".chars()).ok(), Some(vec!['1','2','3','4']));
/// assert_eq!(any.many1::<String>().parse_easy("1234".chars()).ok(), Some("1234".to_string()));
/// assert_eq!(char('a').many1::<String>().parse_easy("aaabaaba".chars()).ok(), Some("aaa".to_string()));
/// assert_eq!(char('a').many1::<String>().parse_easy("".chars()).ok(), None);
/// ```
...
/// use chasa::*;
/// assert_eq!(
///     any.many_with(|iter| iter.enumerate().collect::<Vec<_>>()).parse_easy("abcde".chars()).ok(),
///     Some(vec![(0,'a'),(1,'b'),(2,'c'),(3,'d'),(4,'e')])
/// );
/// assert_eq!(
///     any.many_with(|iter| iter.take(2).collect::<String>()).and(char('c')).parse_easy("abcde".chars()).ok(),
///     Some(("ab".to_string(), 'c'))
/// );
/// assert_eq!(
///     any.and(char('a')).many_with(|iter| iter.collect::<Vec<_>>()).parse_easy("baca".chars()).ok(),
///     Some(vec![('b','a'),('c','a')])
/// );
/// assert_eq!(
///     any.and(char('a')).many_with(|iter| iter.collect::<Vec<_>>()).parse_easy("bacahh".chars()).ok(),
///     None
/// );
/// ```
#[derive(Clone, Copy)]
pub struct ManyWith<P, F>(pub(crate) P, pub(crate) F);
impl<I: Input + Clone, C, S: Clone, M: Cb, P: Parser<I, C, S, M>, F: FnOnce(ParserIterator<prim::RefParser<P>, I, C, S, M>) -> O, O>
...
impl<I: Input + Clone, C, S: Clone, M: Cb, P: Parser<I, C, S, M>, F: Fn(ParserIterator<prim::RefParser<P>, I, C, S, M>) -> O, O> Parser<I, C, S, M>
    for ManyWith<P, F>
{
    #[inline]
    fn run(&self, cont: ICont<I, C, S, M>) -> IResult<O, I, S, M> {
...
/// an iterator that returns `P::Output`. It can only be used locally within `many_with`.
